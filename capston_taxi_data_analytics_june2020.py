# -*- coding: utf-8 -*-
"""Capston_Taxi_Data_Analytics_June2020.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ekuOgC-0GD58nQw4A5UK2FiaDxo7JZyb
...
# yellow taxi data
file_loc = '/content/drive/MyDrive/Onelearn_Data_Sciece_Course/EDA/Taxi_Project/Capstone_Project_Data/yellow_tripdata_June_2020.parquet'
...
"""* From the above table we can observe missing values in passenger_count and congestion_surcharge for 50718 trips.
...
trip_data.to_parquet('/content/drive/MyDrive/Onelearn_Data_Sciece_Course/EDA/Taxi_Project/Capstone_Project_Data/yellow_tripdata_June_2020_cleaned.parquet',index=False)
...
trip_data = pd.read_parquet('/content/drive/MyDrive/Onelearn_Data_Sciece_Course/EDA/Taxi_Project/Capstone_Project_Data/yellow_tripdata_June_2020_cleaned.parquet')
trip_data.head()
...
"""Since there is only one row with negative duration, we will remove them so as to do our analysis in a better way"""
...
* tip_amount - most of the tip amount is within 0-2.5 dollar as shown in box plot. Though again here too we have outliers, the maximum of which is around 423 dollars. 


* total_taxes - most of the total_taxes values is within 3-6 dollars as shown in box plot. Though we have outliers in this case but it is not as signiificant as the case for tip and fare.


* total_amount - most of the total_amount values is within 10-18 dollars as shown in box plot. Again the outliers in this case seems mostly because of outliers in fare_amount.

* tolls_amount - most of the tolls_amount value is 0 so it seems most of the trips do not have to pay for tolls. Still there are outliers and the maximum toll amount is 114.75 dollars


* duration - most of the values in duration is within 5-14 minutes range as is shown by the median value. We do have some outliers which are beyond the range of 4000 minutes.


* trip_distance - most of the trip_distance is within 1-3 miles value as is shown by the median. Outliers in this case seems mostly because of outliers in duration
...
"""* From above we can understand that most of the payments are done through cash and credit cards. The proportion of credit card payments is around 65% and for cash is around 34%.
...
* Trip Hour 
    ...
    * Peak hour for the pick up is around evening from 12 to 5PM. and peak hours for drop off are between 12 to 6 pm. The busiest time is 3PM.
    * there is less traffic during night times, and around 6-7AM in morning, pickup and drop off starts picking up pace.
* Trip day
    * Tuesday has the highest taxi uses while Sunday has the lowest taxi uses.
    * On weekdays, Monday has the 2nd highest taxi uses after Tuesday, while Wednesday is the lowest.
...
"""The above plots looks quite messy but one insight that we can indetify from above plot that most of pickup and dropoff points do not have more than 1% traffic (1 percent of 549797 total trips is 5498).
...
* The busiest location in terms of pickup are 236, 237, 186, 140, 141, 263, 48
* The busiest location for dropoff too are 236, 237, 75.
...
trip_data.to_parquet('/content/drive/MyDrive/Onelearn_Data_Sciece_Course/EDA/Taxi_Project/Capstone_Project_Data/yellow_tripdata_June_2020_cleaned_before_routes.parquet',index=False)

trip_data=pd.read_parquet('/content/drive/MyDrive/Onelearn_Data_Sciece_Course/EDA/Taxi_Project/Capstone_Project_Data/yellow_tripdata_June_2020_cleaned_before_routes.parquet')
...
# trip_data['routes'].to_csv('/content/drive/MyDrive/Onelearn_Data_Sciece_Course/EDA/Taxi_Project/Capstone_Project_Data/yellow_tripdata_June_2020_cleaned_after_routes.parquet',index=False)

# loading routes_data
file_loc_routes_data = '/content/drive/MyDrive/Onelearn_Data_Sciece_Course/EDA/Taxi_Project/Capstone_Project_Data/yellow_tripdata_June_2020_cleaned_before_routes.parquet'
trip_data = pd.read_parquet(file_loc_routes_data)
# assigning new column 'routes' in trip_data to routes_data
trip_data.head()

trip_data_routes = pd.read_csv('/content/drive/MyDrive/Onelearn_Data_Sciece_Course/EDA/Taxi_Project/Capstone_Project_Data/yellow_tripdata_June_2020_cleaned_after_routes.parquet')
trip_data['routes'] = trip_data_routes

"""Now let's explore routes through the same bar plot code that we used for Location ID's. But in this case we will only look for routes with more than 0.25% counts (1375 trips)."""
...
"""* From the above plot we can observe that most of the hours have outliers in fare based on pickup time and dropoff time.
...
"""* We can see from the plots that for pickup the median fare_amount is slightly higher during 12AM-4AM and while for dropoff hours median fare_amount is almost same for all the hours. Though there is variability in during 3:00 - 6:00 AM.
...
"""* Again we can see the median value of total_amount for early morning hours (12AM-4AM) are higher than the rest of the hours for pickup. And some variability can be observed between 4AM-7AM.

* The median value of total_amount for drop off is almost same for all the hours, though some variability can be observed between 4AM-8AM.
...
restricted_total_taxes_data.info()
...
restricted_tip_amount_data_morning_hours_dropoff=restricted_tip_amount_data.loc[(restricted_tip_amount_data["trip_dropoff_hour"].isin([0,1,2,3,4,5])),['trip_dropoff_hour','tip_amount']]
restricted_tip_amount_data_morning_hours_dropoff.groupby(['trip_dropoff_hour']).agg(["max","min",'count',"median",'mean'])

restricted_tip_amount_data_morning_hours_pickup=restricted_tip_amount_data.loc[(restricted_tip_amount_data["trip_pickup_hour"].isin([0,1,2,3,4,5])),['trip_pickup_hour','tip_amount']]
restricted_tip_amount_data_morning_hours_pickup.groupby(['trip_pickup_hour']).agg(["max","min",'count',"median",'mean'])

"""* Based on tip_amount plot we can see that median tip_amount from 6AM to 23PM have some variability for pickup and drop off. while for early morning hours (12AM-5AM) median is zero
...
restricted_total_taxes_data_dropoff=restricted_total_taxes_data.loc[:,['trip_dropoff_hour','total_taxes']]
restricted_total_taxes_data_dropoff

restricted_total_taxes_data_dropoff.groupby(['trip_dropoff_hour']).agg(["max","min",'count',"median",'mean'])

restricted_total_taxes_data_pickup=restricted_total_taxes_data.loc[:,['trip_pickup_hour','total_taxes']]
restricted_total_taxes_data_pickup.groupby(['trip_pickup_hour']).agg(["max","min",'count',"median",'mean'])

"""Now in this plot we can clearly observe that total_taxes change with hour of the day. There are three patterns that we can observe here:<br>
* From 8PM to 5AM have the median total_taxes is same, while during evening hours (4PM-7PM) they are slightly higher for dropoff and pickup, While during 6AM-3PM the median total_taxes are lowest.
...
pickup_location_fare_amount_describe_sorted_count=pickup_location_fare_amount_describe[("fare_amount","count")].sort_values(ascending=False)
sorted_index_list=pickup_location_fare_amount_describe_sorted_count.index
pickup_location_fare_amount_describe_sorted_count_True=pickup_location_fare_amount_describe.reindex(sorted_index_list).reset_index()
pickup_location_fare_amount_describe_sorted_count_True.drop(['index'],inplace=True,axis=1)
pickup_location_fare_amount_describe_sorted_count_True

"""* So from above plot we can observe that for busiest pickup location median fare_amount is quite low. Though the outliers for pickup location 204 is high.
...
# import numpy as np
# import seaborn as sns
# restricted_duration=pd.read_parquet('/content/drive/MyDrive/Onelearn_Data_Sciece_Course/EDA/Taxi_Project/Capstone_Project_Data/yellow_tripdata_June_2020_cleaned.parquet')
...
* The median for late night hours 12AM-4AM is slightly higher compared to others
...
pickup_location_duration_describe_sorted_count=pickup_location_duration_describe[("duration","count")].sort_values(ascending=False)
sorted_index_list=pickup_location_duration_describe_sorted_count.index
pickup_location_duration_describe_sorted_count_True=pickup_location_duration_describe.reindex(sorted_index_list).reset_index()
pickup_location_duration_describe_sorted_count_True.drop(['index'],inplace=True,axis=1)
pickup_location_duration_describe_sorted_count_True

"""* Here again we can see for top 3 most frequent pickup locations 236, 237, 186 the duration value is less in comparison to other pickup locations. They have 7.65, 7.2. 8.95 median duration. 

* While 204, 44, 86, 217, 221 are the top 5 locations with highest median duration.
...
* We already observed that most of the trips are within 5-14 minutes range, so keeping these values to create bins
"""

print(Duration_Fare_Taxes.shape)
Duration_Fare_Taxes=Duration_Fare_Taxes.loc[((Duration_Fare_Taxes["duration"]>=5) & (Duration_Fare_Taxes["duration"]<=14))].copy()
print(Duration_Fare_Taxes.shape)
Duration_Fare_Taxes

bin=['5-6', '6-7', '7-8', '8-9', '9-10', '10-11','11-12','12-13','13-14']
Duration_Fare_Taxes['duration_bin']=pd.cut(Duration_Fare_Taxes["duration"], range(5,15,1),labels=bin)
Duration_Fare_Taxes
...
"""* We already found that the busiest location in terms of pickup are 236, 237, 186, 140, 141, 263, 48 and busiest location in terms of dropoff are 236, 237, 75.
* We also already found that peak hours of pick up are 12 to 5PM, and peak hours for drop off are 12-6 pm. And busiest time is 3PM.
* So we will focus our analysis on these locations and hours only
"""

print(Fare_taxes_location_time.shape)
Fare_taxes_location_time=Fare_taxes_location_time.loc[Fare_taxes_location_time["PULocationID"].isin([236,237,186,140,141,263,48])]
print('After selecting pickup location',Fare_taxes_location_time.shape)
Fare_taxes_location_time=Fare_taxes_location_time.loc[Fare_taxes_location_time["DOLocationID"].isin([236,237,75])]
print('After selecting dropoff location',Fare_taxes_location_time.shape)
Fare_taxes_location_time=Fare_taxes_location_time.loc[Fare_taxes_location_time["trip_pickup_hour"].isin([12,13,14,15,16,17])]
print('After selecting pickup hour location',Fare_taxes_location_time.shape)
Fare_taxes_location_time=Fare_taxes_location_time.loc[Fare_taxes_location_time["trip_dropoff_hour"].isin([12,13,14,15,16,17,18])]
...
"""* Busiest location in terms of pickup were 236, 237, 186, 140, 141, 263, 48 in descending order, but maximum fare_amount are found for 186 and 48 pickup location
* Busiest location in terms of dropoff were 236, 237, 75 in descending order, but maximum fare_amount is onserved for location 75
* Total_tax is found to be same most if the pickup and dropoff locations, though for location 186 total_tax for pickup is higher for 2PM and 5PM
...
Duration_location_time

print(Duration_location_time.shape)
Duration_location_time=Duration_location_time.loc[Duration_location_time["PULocationID"].isin([236,237,186,140,141,263,48])]
print('After selecting pickup location',Duration_location_time.shape)
Duration_location_time=Duration_location_time.loc[Duration_location_time["DOLocationID"].isin([236,237,75])]
print('After selecting dropoff location',Duration_location_time.shape)
Duration_location_time=Duration_location_time.loc[Duration_location_time["trip_pickup_hour"].isin([12,13,14,15,16,17])]
print('After selecting pickup hour location',Duration_location_time.shape)
Duration_location_time=Duration_location_time.loc[Duration_location_time["trip_dropoff_hour"].isin([12,13,14,15,16,17,18])]
...
"""* Busiest location in terms of pickup were 236, 237, 186, 140, 141, 263, 48 in descending order, but maximum trip duration are found for 186 and 48 pickup location
* Busiest location in terms of dropoff were 236, 237, 75 in descending order, but maximum trip duration are observed for location 75 during 12PM-5PM, but during 6PM maximum trip duration is observed for location 236.

## FINAL RESULTS FROM EDA

Following insights would be useful for
...
* Most of the hours have outliers in fare based on pickup time and dropoff time

* For pickup the median fare_amount is slightly higher during 12AM-4AM and while for dropoff hours median fare_amount is almost same for all the hours. Though there is variability in during 3:00 - 6:00 AM.
...
* Top 3 most frequent pickup locations 236, 237, 186 the duration value is less in comparison to other pickup locations. They have 7.65, 7.2. 8.95 median duration. 
...
* Busiest location in terms of pickup were 236, 237, 186, 140, 141, 263, 48 in descending order, but maximum fare_amount are found for 186 and 48 pickup location for all peak hours

* Busiest location in terms of dropoff were 236, 237, 75 in descending order, but maximum fare_amount is onserved for location 75 for all peak hours

* Total_tax is found to be same, though for location 186 total_tax for pickup is higher for 2PM and 5PM 

* Busiest location in terms of pickup were 236, 237, 186, 140, 141, 263, 48 in descending order, but maximum trip duration are found for 186 and 48 pickup location for all peak hours

* Busiest location in terms of dropoff were 236, 237, 75 in descending order, but maximum trip duration are observed for location 75 during 12PM-5PM, but during 6PM maximum trip duration is observed for location 236 for all peak hours
"""